...
class CommonGenDatasetP(Dataset):

	def __init__(self, config, json_path, tokenizer, copy_vocab, decoder_start_token_id, is_training=False,
				 attachable_index=None):
		super(CommonGenDatasetP, self).__init__()
	...
	def read_content(self, json_path):  # json文件 有几个概念词，然后有三句用概念词造的句子。
		...
		with open(json_path) as out:
			...
			for l in tqdm(lines):
				...
				expected_input_len = len(self.tokenizer('生成文案 : ' + ','.join(item['concept_set'].split('#')))['input_ids'])
				expected_output_len = len(self.tokenizer(item['scene'])['input_ids'][0])
				# 这边输入的keyword都会被替换成copy vocab里头的第一个，替换后如果文中没有出现，会导致在文中找不到词语，数据被丢掉的情况。
				if expected_input_len > keyword_len:
					continue
				if expected_output_len > 512:
					continue

				concept_set_input_ids = self.tokenizer(concept_set, return_tensors="np")['input_ids'][0,
										:-1].tolist()  # input_ids
				concept_cls = []  # keyword的category id，也就是copy vocab中的行号
				...
				start_pos = []  # 每个keyword在input里头的开始位置
				...
				position_indicator = np.zeros((keyword_len, len(concept_set_input_ids)), dtype=np.float32)
				for i in range(len(concept_cls)):
					position_indicator[i, start_pos[i]:start_pos[i + 1]] = 1  # 关键词位置赋值为1
				sum_check = np.sum(position_indicator, axis=1)
				for i in range(len(concept_cls)):
					assert sum_check[i] > 0

				cls_on_input = np.zeros((len(concept_set_input_ids),), dtype=np.int64)  # 关键词的span上放上category id
				for i, cls_ in enumerate(concept_cls):
					cls_on_input[start_pos[i]:start_pos[i + 1]] = cls_

				gt = copy.deepcopy(item['scene'])  # reference sequences
				if self.is_training:
					for c in item['scene']:
						c = c.lower()
						...
						if self.config.use_pointer:
							for c_cls in concept_cls:
								for _, fg_index in self.copy_vocab.d_to_w_group[c_cls]:
									...
									string_caption = re.sub(' %s ' % fg_str, ' (%d) ' % fg_softmax_index,
															string_caption)
									string_caption = re.sub(' %s$' % fg_str, ' (%d)' % fg_softmax_index, string_caption)
									string_caption = re.sub('^%s ' % fg_str, '(%d) ' % fg_softmax_index, string_caption)

						c_input_ids = []
						str_id_list = string_caption.split()  # labels
						copy_mention_flag = np.zeros((len(str_id_list) + 1, keyword_len))
						decoder_mention_flag = np.zeros(
							(len(str_id_list), len(concept_set_input_ids)))  # [输出序列维度，输入序列维度]
						...
						if self.config.use_pointer:
							...
							for index, w in enumerate(str_id_list):
								c_input_ids.append(int(w))
								assert c_input_ids[-1] <= self.config.vocab_size  # 确保token id在词表范围内
								...
								for (_, fg_index) in self.copy_vocab.d_to_w_group[cls_index]:
									...
									for ch_idx, first_ch in enumerate(c_input_ids):
										if first_ch == fg_ch_list[0]:
											s2 = '&'.join(
												[str(f) for f in c_input_ids[ch_idx: ch_idx + len(fg_ch_list)]])
											if s1 == s2:  # 关键词和句子匹配上了
												# if ch_idx + len(fg_ch_list) >= len(c_input_ids) - 1 or c_input_ids[
												# 	ch_idx + len(fg_ch_list)] not in self.attachable_index: # 判断当前词语是不是已经结尾了，如果是string的结尾或者后一个词不可能和当前词语组成一个完整的词语的话(以__开头)，那么可以认为当前词语已经结束，而因为s1 == s2，所以将当前数据加入训练集。
												decoder_mention_flag[:ch_idx + len(fg_ch_list),
							...
							if not self.config.static_mf:
								for j in range(len(concept_cls)):
									for jj in range(start_pos[j], start_pos[j + 1]):
										if not decoder_mention_flag[-1, jj] == 2:  # 如果有关键词没有出现的，就跳过样例
											use_this_record = False

						if use_this_record:  # 没有被跳过的话就把样例放到instance_tuple里头去
							instance_tuple = (
							concept_set_input_ids, position_indicator, cls_on_input, concept_cls, copy_mention_flag,
							decoder_mention_flag, c_input_ids, gt, item['concept_set'].split('#'))
							self.record.append(instance_tuple)

				else:  # 如果不是训练模式
					copy_mention_flag = np.zeros((1, keyword_len))
					...
					self.record.append((concept_set_input_ids, position_indicator, cls_on_input, concept_cls,
										copy_mention_flag, decoder_mention_flag, None, gt,
										item['concept_set'].split('#')))
	...
	def __getitem__(self, index):
		concept_set, pos, cls_on_input, concept_cls, copy_mention_flag, decoder_mention_flag, gen, gt, gt_concept = \
		self.record[index]
	...
	def read_content(self, json_path): # json文件 有几个概念词，然后有三句用概念词造的句子。
		...
		with open(json_path) as out:
			...
			for l in tqdm(lines):
				...
				concept_set_input_ids = self.tokenizer(concept_set, return_tensors="np")['input_ids'][0, :-1].tolist() # input_ids
				concept_cls = [] # keyword的category id，也就是copy vocab中的行号
				for concept in item['concept_set'].split('#'):
					if concept in self.copy_vocab.word_to_category_id: # 如果输入的词语在word_to_category_id里头，就在concept_cls放word_to_category_id的id
						concept_cls.append(self.copy_vocab.word_to_category_id[concept])
				...
				if self.is_training:
					for c in item['scene']:
						...
						decoder_mention_flag = np.zeros((len(str_id_list), len(concept_set_input_ids))) # [输出序列维度，输入序列维度]
						use_this_record = True
						if self.config.use_pointer:
							for index, w in enumerate(str_id_list):
								if w.startswith('(') and w.endswith(')'):
							...
							for j, cls_index in enumerate(concept_cls):
								if cls_index == 0: continue
								'''
								如果按组的话，拿到一个concept，会在文本搜索同一组的所有词有没有在文中出现。
								但是如果不按组构建copy_vocab的话，如果concept和文本中不一样，就检测不到。
								'''
								for (_, fg_index) in self.copy_vocab.d_to_w_group[cls_index]: # 这边是遍历相似的词语
									fg_ch_list = self.copy_vocab.token_fg_w[fg_index]
									s1 = '&'.join([str(f) for f in fg_ch_list])
									...
									for ch_idx, first_ch in enumerate(c_input_ids):
										if first_ch == fg_ch_list[0]:
											...
											if s1 == s2: # 关键词和句子匹配上了
												# if ch_idx + len(fg_ch_list) >= len(c_input_ids) - 1 or c_input_ids[ch_idx + len(fg_ch_list)] not in self.attachable_index: # attachable_index 里面放的是可以自己开头的词语，也就是以__开头的，这边是要判断是不是当前词语的结尾。如果后一个词可以自己开头，则可以判定当前词语不是别的某个词语的前缀。
...
def data_wrapper(dataset, tokenizer, decoder_start_token_id):
	...
	copy_pos = np.zeros((batch_size, keyword_len, max_concept_len), dtype=np.float32)
	for i, d in enumerate(dataset):
		copy_pos[i, :, :d['copy_pos'].shape[-1]] = d['copy_pos']
	new_dataset['copy_pos'] = torch.from_numpy(copy_pos)

	concept_cls = np.zeros((batch_size, keyword_len), dtype=np.int64)
	for i, d in enumerate(dataset):
	...
	copy_mention_flag = np.zeros((batch_size, max_gen_len, keyword_len), dtype=np.int64)
	for i, d in enumerate(dataset):
