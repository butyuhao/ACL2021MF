        ...
        for epoch in range(epoch_num):
            ...
            with tqdm(total=math.ceil(run_step / _C.gradient_accumulation_steps), file=sys.stdout) as pbar:
                for step in range(run_step):
                    ...
                    outputs = model(
                        input_ids=batch['input_ids'], # [bs, 16]
                        attention_mask=batch['attention_mask'], 
                        decoder_copy_pos=batch['copy_pos'], # 对于每条数据有[bs, 5, 16]的tensor，每行在出现需要copy的词的时候为1。
                        decoder_concept_cls=batch['concept_cls'], # [bs, 5] cls_on_input 的后5个，去掉前面prompt的部分
                        decoder_input_ids=batch['decoder_input_ids'], # [bs, 25]
                        decoder_attention_mask=batch['decoder_input_mask'],
                        decoder_copy_mention_flag=batch['copy_mention_flag'], # [bs, 25, 5] # 在input concept的位置为1，其它为0
                        decoder_mention_flag=batch['decoder_mention_flag'], # [bs, 25, 16] 文章中提到的mf
                        decoder_cls_on_input=batch['cls_on_input'], # [bs, 16] 输入中concpet位置上是对应的词组行号
